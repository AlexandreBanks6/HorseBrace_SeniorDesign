...
// Stop at the end of recording writes sector number to the eeprom. Play back reads
// eeprom and plays to that sector number.  
...
unsigned long endloc=0xFFFFFFFF;	//stop location0xFFFFFFFF
...
void InitSD(void)
{
	...
	i=100;	//try idle state for 100 times
...
void WriteSD(void)	
{
	...
	//write end location
	eeprom_write(1,(loc & 0xFF000000) >> 24);
	eeprom_write(2,(loc & 0x00FF0000) >> 16);
	eeprom_write(3,(loc & 0x0000FF00) >> 8);
	eeprom_write(4,loc & 0x000000FF);
...
void ReadSD(void)
{
	...
	//	read end location from eeprom
	endloc = eeprom_read(4) | ( (unsigned long)eeprom_read(3) << 8 ) | ( (unsigned long)eeprom_read(2) << 16 ) | ( (unsigned long)eeprom_read(1) << 24 );
	playLED=1;
	CS = 0;
...
void main(void)
{
	...
	// PIC I/O init
	TRISC = 0b10000;		// sck rc3, sdo rc5, sdi rc4, CS rc0, rc2 CPP1.
